...
if(isset($_SESSION['WMS_CLASSIFICATION']))
{
	$class = $_SESSION['WMS_CLASSIFICATION'];
} else {
	$class = "";
}
...
function getRecipientData()
{
	...
	$.post("./Modules/Warehouse_Management/includes/item_classifications_data.php", { limit: limit, recipient: recipient },
	function(data) {		
		$('#smnavdata').html(data);
		rms_reloaderOff();
	});
...
$(function()
{
	...
	$('#search').keyup(function()
	{
		...
		$.post("./Modules/Warehouse_Management/includes/item_classifications_data.php", { limit: limit, search: search, recipient: recipient, ord: ord },
		function(data) {
			$('#smnavdata').html(data);
		});
...
function load_data()
{
	...
	setTimeout(function()
	{
		$.post("./Modules/Warehouse_Management/includes/item_classifications_data.php", { limit: limit, recipient: recipient, ord: ord },
		function(data) {
			$('#smnavdata').html(data);
			rms_reloaderOff();
		});
