...
';
$RMShtml .='
				<div style="border-bottom-style:dotted; border-color: grey; margin-bottom: 5px;padding-bottom:20px ">
				<div style="margin-bottom:10px">	<span style="font-weight:600;font-size:14px">'.$branch.'</span> | </span> <span style="font-size:14px">MRS No: '.$control_no.'</span> | <span style="font-weight:normal;font-size:14px">Order Date: '.date("Y-m-d", strtotime($function->GetOrderDate($control_no,$branch,$db))).'</span></div>
';
		/* ####################################################################################################### */			
		//	$query = "SELECT * FROM wms_picklist WHERE branch='$branch' AND trans_date='$trans_date'";
			$query = "SELECT * FROM wms_picklist WHERE control_no='$control_no'";
